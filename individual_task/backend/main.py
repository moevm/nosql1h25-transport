...
async def get_customers(
    ...
    if type_contact_information:
        query["contact_information"] = type_contact_information
    if type_address:
        query["address"] = type_address
...
async def get_tcs(
    ...
    query = {}
    if type_tcs:
        query["type_tc"] = type_tcs
    if type_registration_number:
        query["registration_number"] = type_registration_number
...
async def get_rents(
    ...
    type_date_of_begining: Optional[str] = Query(None),
    type_date_of_ending: Optional[str] = Query(None)
):
    query = {}
    if type_order_name:
        query["order_name"] = type_order_name
    if type_registration_number:
        query["registration_number"] = type_registration_number
    if type_rent_id:
        query["rent_id"] = type_rent_id
    if type_date_of_begining:
        query["date_of_begining"] = type_date_of_begining
    if type_date_of_ending:
        query["date_of_ending"] = type_date_of_ending
