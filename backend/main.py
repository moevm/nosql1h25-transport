...
async def get_cars(
    ...
    type_year_from: Optional[int] = Query(None),
    type_year_to: Optional[int] = Query(None)
):
    query = {}
    ...
    if type_year_from:
        query["year"] = {"$regex": type_year_from, "$options": "i"}
    if type_year_to:
        query["year"] = {"$regex": type_year_to, "$options": "i"}
